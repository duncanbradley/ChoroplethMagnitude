...
**Choropleth mapping employs datasets where each data point corresponds to a discrete area, typically defined by administrative boundaries (e.g., national or local government regions). Ratios, proportions and averages are often plotted to enable appropriate comparisons (Dent et al., 2008). Dent et al. (2008) discuss several considerations for choropleth map design, including data pre-processing, spatial detail, and appropriate textual accompaniments. However, data classification is a particularly prominent theme in work choropleth map design.** 

**Decisions around classification involve trade-offs between legibility of spatial patterns in the map and legibility of the legend. Jenks’ Natural Breaks algorithm identifies class boundaries according to the distribution of data, ensuring clusters of similar values appear homogenous. The Equal Frequency method ensures uniform prevalence of each class within the map, whereas the Equal Interval method simply employs the same numerical range for each class (Dent, 2008). Unclassed choropleth maps (Tobler, 1973) do not employ discrete groups at all. Instead, each unique value is represented distinctly. This may increase estimation error, yet avoids the impression that similar values either side of a class boundary are substantially different (Kraak and Ormelling, 2011).**
